...
function loadIndex(): IndexMap { try { return JSON.parse(fs.readFileSync(INDEX_FILE, 'utf8')); } catch { return {}; } }
function saveIndex(ix: IndexMap) { fs.writeFileSync(INDEX_FILE, JSON.stringify(ix), 'utf8'); }
function keyFor(url: string) { return createHash('sha1').update(url).digest('hex'); }
function sizeOf(file: string) { try { return fs.statSync(file).size; } catch { return 0; } }
function totalSize(ix: IndexMap) { return Object.values(ix).reduce((a, r) => a + (r.s||0), 0); }
