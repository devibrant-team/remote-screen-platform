...
function Spinner({ className = "w-5 h-5 text-red-600" }: { className?: string }) {
  return (
...
function IconCheck({ className = "w-5 h-5 text-green-600" }: { className?: string }) {
  return (
...
export default function CreateScreen() {
  ...
  useEffect(() => {
    ...
    const linked = Boolean(deviceState?.linked) || localStorage.getItem(LS_LINKED) === "1";
  ...
  useEffect(() => {
    ...
    setDeviceState((prev) => ({ ...prev, screenId: screenIdStr, code: data.code }));
  ...
  useEffect(() => {
    const id =
      ...
      (deviceState?.screenId != null ? String(deviceState.screenId) : undefined);
    expectedIdRef.current = id;
  }, [data?.screenId, deviceState?.screenId]);

  /* 6) Echo listener (pairing) */
  const pairingCode = (data?.code ?? deviceState?.code) as string | number | undefined;
  ...
  useEffect(() => {
    ...
    const handler = (e: { screen_id?: number | string; next_url?: string; screenToken?: string  ; screenName?: string }) => {
      ...
      const accept = (expected && eventId === expected) || (!eventId && !!e?.screenToken);
  ...
  const isLinked = Boolean(deviceState?.linked) || lsLinked || linkedRef.current;
      ...
      <div className="flex-1 w-full max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-10">
        ...
        <div className="mb-8 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl sm:text-3xl font-semibold">Register Screen</h1>
              <p className="text-gray-600 text-sm sm:text-base">Link this device to your signage account</p>
            </div>
          </div>
          ...
          <div className="flex items-center flex-wrap gap-2">
            ...
            {!isPending && !isError && !hasId && <Badge color="blue">Awaiting registration</Badge>}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
          ...
          <div className="lg:col-span-3 border border-gray-200 bg-white rounded-2xl p-5 sm:p-6 shadow">
            ...
            <div className="mt-4 grid gap-3 text-sm sm:text-base">
              ...
              <div className="flex items-center justify-between rounded-lg border border-gray-200 px-3 sm:px-4 py-2.5 sm:py-3">
                ...
                <span className="font-mono break-all">
                  {lsScreenId || deviceState?.screenId || <span className="text-gray-400">—</span>}
                </span>
              </div>
            ...
            {showPairingUI && isError && (
              <div className="mt-6 rounded-xl border border-red-200 bg-red-50 p-4 text-sm sm:text-base">
                ...
                <p className="text-red-600 mt-1">{(error as any)?.message ?? "Unknown error"}</p>
            ...
            {showPairingUI && !isError && isPending && (
              <p className="mt-6 text-gray-600 text-sm sm:text-base">Contacting server…</p>
            )}
          </div>
          ...
          <div className="lg:col-span-2 border border-gray-200 bg-white rounded-2xl p-5 sm:p-6 shadow">
            ...
            {showPairingUI && pairingCode != null ? (
              <>
                <p className="mt-2 text-gray-600 text-sm sm:text-base">Your pairing code</p>
              ...
              <div className="mt-3 space-y-3">
                <p className="text-gray-600 text-sm sm:text-base">Waiting for pairing code…</p>
                <div className="flex items-center gap-2 text-gray-500 text-sm">
                  <Spinner className="w-4 h-4 text-red-600" />
                  <span>If this takes long, click “Clear pairing” and try again.</span>
                </div>
              </div>
            )}
        ...
        <div className="mt-10 border border-gray-200 bg-white rounded-2xl p-5 sm:p-6 shadow text-gray-800">
          ...
          <ol className="mt-3 grid gap-3 sm:gap-4 text-sm sm:text-base md:grid-cols-3 text-gray-700">
            <li className="rounded-lg bg-gray-50 p-3 sm:p-4">1) We request a new Screen ID from the server.</li>
            <li className="rounded-lg bg-gray-50 p-3 sm:p-4">2) Enter the pairing code in your admin panel to link this device.</li>
            <li className="rounded-lg bg-gray-50 p-3 sm:p-4">3) When linked, the device saves the token and redirects to the player.</li>
          </ol>
        </div>
      </div>
    </div>
